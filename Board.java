...
public class Board {
  ...
  public Board() {
    ...
    // Fills edges of board with "#"'s to help with edge cases
    for (int j = 0; j < 10; j++) {
      for (int i = 0; i < 10; i++) {
        board[0][j] = 3; // top
        board[9][j] = 3; // bottom
  ...
  /**
   * takes in user input @param x, @param y, and then checks for valid moves, if
   * the move is valid, the board updates accordingly
   */
   ...
   * @return flipped returns a nine integer array, first element is total number
   *         of pieces that would be flipped, the second is the number of pieces
