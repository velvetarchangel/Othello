...
public class OthelloGraphics extends Application{
      ...
      if ((mouseX > 25) && (mouseX < 825) && (mouseY > 25) && (mouseY < 825)){
        ...
        // if board is not filled continue the game
            ...
            if (board.gameOver() == false) {
                ...
                if ((player == 1) && board.AnyMovesLeft("1")) { // Player 1's turn
                    System.out.println("X: " + x + "    Y: " + y);
                    board.printBoard();
                    System.out.println("\n" + "It is player " + player + "'s turn");
                    flipped = board.move(x, y, Integer.toString(player));
                    ...
                    if (flipped[0] != 0)
                    {
                        board.updateBoard(x, y, Integer.toString(player), flipped);
                        board.printBoard();
                ...
                // checks it if it is player 2's turn and runs through the game sequence
                else if ((player == 2) && board.AnyMovesLeft("2")) {
                    board.printBoard();
                    System.out.println("\n" + "It is player " + player + "'s turn");
                    flipped = board.move(x, y, Integer.toString(player));
                    ...
                    else if (flipped[0] != 0)
                    {
                        board.updateBoard(x, y, Integer.toString(player), flipped);
                        board.printBoard();
                        playerTwoScore = board.turnScore(Integer.toString(player));
                        playerOneScore = board.turnScore("1");
                        player_2.setScore(playerTwoScore);
                        player_1.setScore(playerOneScore);
                        System.out.println("\n" + "Player 1's score is " + playerOneScore);
  ...
  */
  public void initMenuBack(){
    ...
    Text title = new Text(200,350,"Othello");
    title.setFont(Font.font("impact",FontWeight.NORMAL,FontPosture.REGULAR,250));
    Text title2 = new Text(560,400,"T08  G05");
  ...
  public void initMenuButtons(){
    ...
    Text startText = new Text(450,520,"Start Game");
  ...
  */
  public void initBoardGrid(){
    ...
    for (int i = 0; i <= 7; i++){
      Line vertical = new Line(100*i+25, 25, 100*i+ 25,825);
      graphicBoard.getChildren().add(vertical);
    }
  ...
  */
  public void initPlayerInfo(){
    Text player1Text = new Text(925,100,"Player 1");
    player1Text.setFont(Font.font("impact",FontWeight.NORMAL,FontPosture.REGULAR,25));
    Text player1Score = new Text(925,200,String.valueOf(playerOneScore));
    player1Score.setFont(Font.font("impact",FontWeight.BOLD,FontPosture.REGULAR,25));
    Text player2Text = new Text(925,300,"Player 2");
  ...
  public void createMessageBoard(){
    ...
    String colour = "";
    if (player == 1) colour = "Black";
    else if (player == 2) colour = "White";
    Text playerText = new Text(945,560, colour+ "'s Turn!");
...
// Creates a stage and adds the group to the scene which is added to the stage and shown on the screen
  @Override
  public void start(Stage mstage) throws FileNotFoundException, InvocationTargetException{
    stage = mstage;
    scene.setRoot(mainMenu());
    scene.setOnMouseClicked(menuHandler);
